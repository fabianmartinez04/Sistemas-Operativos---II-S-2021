...
void runThread(int direction, Thread *thread) {
    ...
    while(!exit) {
        ...
        case UP:
            ...
            // mutex.h
            else if (!game.cell[(lastStep->row-1)*rowsN + lastStep->column].up) {
        ...
        case DOWN:
            ...
            else if (!game.cell[lastStep->row+1*rowsN + lastStep->column].down) {
                game.cell[(lastStep->row+1)*rowsN + lastStep->column].up = 1;
                next->row = lastStep->row + 1;
                lastStep->next = next;
            } else {
        ...
        case LEFT:
            ...
            if (!game.cell[lastStep->row*rowsN + lastStep->column - 1].left) {
        ...
        case RIGHT:
            ...
            if (!game.cell[lastStep->row*rowsN + lastStep->column+1].right) {
                next->column = lastStep->column + 1;
                lastStep->next = next;
            } else {
                exit = 1;
